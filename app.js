...
function initFieldAI() {
  ...
  if (SpeechRecognition) {
    ...
    recognition.addEventListener("result", (event) => {
      fieldAiText.value = `${fieldAiText.value} ${event.results[0][0].transcript}`.trim();
      fieldAiSpeechStatus.textContent = "Áudio transcrito. Clique em Interpretar relato para revisar.";
    });
    recognition.addEventListener("end", () => fieldAiRecord.textContent = "Gravar áudio");
  } else {
  ...
  fieldAiRecord.addEventListener("click", () => {
    ...
    fieldAiRecord.textContent = "Ouvindo...";
    fieldAiSpeechStatus.textContent = "Gravando áudio pelo navegador...";
    recognition.start();
  });
  fieldAiInterpret.addEventListener("click", () => fillFieldAIPreview(interpretFieldAIReport(fieldAiText.value)));
